...
class Base(object):
    ...
    def isApproximatelyEqual(self, other):
        ...
        if self._pointCount != len(other.points): return False
        if self._featureCount != len(other.features): return False
        ...
        if level > 0:
            if self._pointNamesCreated():
                for key in self.points.getNames():
                    assert self.points.getName(self.points.getIndex(key)) == key
            if self._featureNamesCreated():
                for key in self.features.getNames():
                    assert self.features.getName(self.features.getIndex(key)) == key
    ...
    def __repr__(self):
        ...
        ret += dataHelpers.makeNamesLines(indent, maxW, numRows, self._pointCount,
                                          self.points.getNames(), 'pointNames')
        ...
        if byLine:
            ...
            strLength = len("___".join(self.features.getNames())) + \
                        len(''.join([str(i) for i in range(self._featureCount)]))
        ...
        if numCols <= self._featureCount:
            numCols += 1
        ret += dataHelpers.makeNamesLines(indent, maxW, numCols, self._featureCount,
    ...
    def _copyAs_implementation_base(self, format, rowsArePoints, outputAs1D):
        ...
        if not rowsArePoints:
            if format in ['List', 'Matrix', 'Sparse', 'DataFrame']:
                ...
                ret = ret.transpose()
                ret = _createListOfDict(data=ret, featureNames=self.points.getNames())
                return ret
            elif format == 'dictoflist':
                ret = ret.transpose()
                ret = _createDictOfList(data=ret, featureNames=self.points.getNames(), nFeatures=self._pointCount)
                return ret
        ...
        if format == 'listofdict':
            ret = _createListOfDict(data=ret, featureNames=self.features.getNames())
        if format == 'dictoflist':
            ret = _createDictOfList(data=ret, featureNames=self.features.getNames(), nFeatures=self._featureCount)
    ...
    def fillUsingAllData(self, match, fill, arguments=None, points=None,
        ...
        if returnModified:
            ...
            modNames = [name + "_modified" for name in modified.features.getNames()]
    ...
    def _namesAreFlattenFormatConsistent(self, flatAxis, newFLen, newUFLen):
        ...
        """
        flat = self.points.getNames() if flatAxis == 'point' else self.features.getNames()
        formatted = self.features.getNames() if flatAxis == 'point' else self.points.getNames()
    ...
    def unflattenFromOnePoint(self, numPoints):
        ...
        if self._pointCount != 1:
            msg = "Can only unflattenFromOnePoint when there is only one point. " \
                  "This object has " + str(self._pointCount) + " points."
            raise ImproperActionException(msg)
        if self._featureCount % numPoints != 0:
            msg = "The argument numPoints (" + str(numPoints) + ") must be a divisor of " \
                  "this object's featureCount (" + str(self._featureCount) + ") otherwise " \
    ...
    def unflattenFromOneFeature(self, numFeatures):
        ...
        if self._featureCount != 1:
            msg = "Can only unflattenFromOneFeature when there is only one feature. " \
                  "This object has " + str(self._featureCount) + " features."
            raise ImproperActionException(msg)

        if self._pointCount % numFeatures != 0:
            msg = "The argument numFeatures (" + str(numFeatures) + ") must be a divisor of " \
                  "this object's pointCount (" + str(self._pointCount) + ") otherwise " \
                  "it will not be possible to equally divide the elements into the desired " \
                  "number of features."
            raise ArgumentException(msg)
    ...
    def elementwiseMultiply(self, other):
        ...
        self.points.setNames(retPNames)
        self.features.setNames(retFNames)
        self.validate()
    ...
    def __pow__(self, other, z):
        ...
        if other == 0 or other % 2 == 0:
            ...
            identity = UML.createData(self.getTypeString(), numpy.eye(self._pointCount),
    ...
    def _arrangePointNames(self, maxRows, nameLength, rowHolder, nameHold):
        ...
        pnamesWidth = 0
        nameCutIndex = nameLength - len(nameHold)
        (tRowIDs, bRowIDs) = dataHelpers.indicesSplit(maxRows, self._pointCount)
        ...
        for sourceIndex in range(2):
            ...
            if sourceIndex == 1 and len(bRowIDs) + len(tRowIDs) < self._pointCount:
    ...
    def _arrangeDataWithLimits(self, maxWidth, maxHeight, sigDigits=3,
        ...
        (tRowIDs, bRowIDs) = dataHelpers.indicesSplit(maxDataRows, self._pointCount)
    ...
    def _equalPointNames(self, other):
        if other is None or not isinstance(other, Base):
            return False
        return self._equalNames(self.points.getNames(), other.points.getNames())

    def _equalFeatureNames(self, other):
        if other is None or not isinstance(other, Base):
            return False
        return self._equalNames(self.features.getNames(), other.features.getNames())
    ...
    def _validateEqualNames(self, leftAxis, rightAxis, callSym, other):
        ...
        def _validateEqualNames_implementation():
            lnames = self.points.getNames() if leftAxis == 'point' else self.features.getNames()
            rnames = other.points.getNames() if rightAxis == 'point' else other.features.getNames()
    ...
    def _getIndex(self, identifier, axis):
        ...
        nameGetter = self.points.getIndex if axis == 'point' else self.features.getIndex
