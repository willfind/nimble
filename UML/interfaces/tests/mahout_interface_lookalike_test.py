...
def MahoutHandmadeOutput():
    ...
    trialIn.flush()
    args = {'numClusters': '2'}
    UML.trainAndApply("mahout.Kmeans", trainingIn.name, trainY=None, testX=trialIn.name, output=actualOut.name,
                      arguments=args)
...
def MahoutTasteBasedPatchIntegrity():
    ...
    try:
        batchOrig = open(
            getMahoutLocation() + '/core/src/main/java/org/apache/mahout/cf/taste/common/TasteRecommenderBatchEstimation.java')
