...
def test_elementwiseMultiply_callsObjElementsMultiply(mockObj):
    left = [[1, 2, 3], [4, 5, 6]]
    right = [[6, 5, 4], [3, 2, 1]]
    for leftRType in UML.data.available:
        leftObj = UML.createData(leftRType, left)
        for rightRType in UML.data.available:
            rightObj = UML.createData(rightRType, right)
            try:
...
def test_elementwiseMultiply():
    ...
    for leftRType in UML.data.available:
        ...
        expObj = UML.createData(leftRType, exp)
        for rightRType in UML.data.available:
            rightObj = UML.createData(rightRType, right)
...
def test_elementwiseMultiply_logCount():
    left = [[1, 2, 3], [4, 5, 6]]
    right = [[6, 5, 4], [3, 2, 1]]
    leftObj = UML.createData('Matrix', left, useLog=False)
    rightObj = UML.createData('Matrix', right, useLog=False)
    mult = elementwiseMultiply(leftObj, rightObj)
