...
def testMlpyHandmadeSVMClassification():
    ...
    expected = nimble.data('Matrix', [[1], [2]], useLog=False)
...
def testMlpyHandmadeInnerProductTrainingPCA():
    ...
    trainObj = nimble.data('Matrix', kernData, useLog=False)

    data2 = [[4, 4]]
    kernData2 = mlpy.kernel_linear(data2, data2)
    testObj = nimble.data('Matrix', kernData2, useLog=False)
...
def testMlpyKernelLearners():
    ...
    trainingObj4d = nimble.data('Matrix', data4d, featureNames=variables, useLog=False)

    data2d = [[0, 1, 1], [0, 0, 1], [0, 3, 2], [1, -300, 2], [1, 1, 500]]
    trainingObj2d = nimble.data('Matrix', data2d, featureNames=variables, useLog=False)
...
def testMlpyKernelExponentialDisallowed():
    ...
    data2d = [[0, 1, 1], [0, 0, 1], [0, 3, 2], [1, -300, 2], [1, 1, 500]]
    trainingObj2d = nimble.data('Matrix', data2d, featureNames=variables)
...
def test_getScores_exception():
    ...
    data = [[0, 1, 1], [0, 0, 1], [1, 3, 2], [2, -300, 2], [3, 1, 500]]
    trainingObj = nimble.data('Matrix', data)
