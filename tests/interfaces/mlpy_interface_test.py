...
def testMlpyHandmadeSVMClassification():
    ...
    expected = nimble.data([[1], [2]], useLog=False)
...
def testMlpyHandmadePCA():
    ...
    trainingObj = nimble.data(data, useLog=False)
...
def testMlpyHandmadeInnerProductTrainingPCA():
    ...
    trainObj = nimble.data(kernData, useLog=False)

    data2 = [[4, 4]]
    kernData2 = mlpy.kernel_linear(data2, data2)
    testObj = nimble.data(kernData2, useLog=False)
...
def testMlpyOLSDisallowed():
    ...
    variables = ["Y", "x1", "x2"]
    data = [[0, 1, 1], [0, 0, 1], [1, 3, 2], [2, -300, 2], [3, 1, 500]]
    trainingObj = nimble.data(data, featureNames=variables)
...
def testMlpyKernelLearners():
    ...
    trainingObj4d = nimble.data(data4d, featureNames=variables, useLog=False)

    data2d = [[0, 1, 1], [0, 0, 1], [0, 3, 2], [1, -300, 2], [1, 1, 500]]
    trainingObj2d = nimble.data(data2d, featureNames=variables, useLog=False)
...
def testMlpyKernelExponentialDisallowed():
    ...
    data2d = [[0, 1, 1], [0, 0, 1], [0, 3, 2], [1, -300, 2], [1, 1, 500]]
    trainingObj2d = nimble.data(data2d, featureNames=variables)
