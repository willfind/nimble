...
def tableString(table, rowHeader=True, headers=None, roundDigits=None, columnSeperator="", sortColumn=None, maxRowsToShow=None, snipIndex=None, useSpaces=True, includeTrailingNewLine=True):
	...
	if sortColumn != None:
		if isinstance(sortColumn, (str, unicode)): #if we're sorting by the column with a given name
			if headers == None: raise Exception("Cannot find the sortColumn '" + str(sortColumn) + "' since headers=" + str(headers))
			sortColumn = headers.index(sortColumn)
		if not isinstance(sortColumn, (int, long)): raise Exception("sort column must be an integer, but was: " + str(sortColumn))
		if sortColumn < 0: raise Exception("sortColumn should have been a non-negative integer but was: " + str(sortColumn))
		if table[0] == headers:
			tempHeaders = table[0]
			table = table[1:]
		else:
