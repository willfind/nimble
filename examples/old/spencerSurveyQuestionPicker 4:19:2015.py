		...
		for featureToDrop in xrange(allFeatures.featureCount):
			...
			for trainX, trainY in zip(trainXs, trainYs):
				...
				if "Logistic" in algorithmName:
					results = trainAndTest(algorithmName, trainXWithoutFeature, trainY, testX=trainXWithoutFeature, testY=trainY, performanceFunction=rSquared, C=tuple([10.0**k for k in xrange(-6, 6)]))
				elif "Ridge" in algorithmName:
					results = trainAndTest(algorithmName, trainXWithoutFeature, trainY, testX=trainXWithoutFeature, testY=trainY, performanceFunction=rSquared, alpha=tuple([10.0**k for k in xrange(-6, 6)]))
				else:
