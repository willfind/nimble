...
def reduceDataToBestFeatures(trainXs, trainYs, testXs, testYs, numFeaturesToKeep, predictionAlgorithms, featuresToPredict):
...
def getBestFeaturesAndAccuracies(data, numFeaturesToKeep, predictionAlgorithms, fractionOfDataForTesting, featuresToPredict, functionsToExcludePoints):
	
	trainXs, trainYs, testXs, testYs = buildTrainingAndTestingSetsForPredictions(data, fractionOfDataForTesting=fractionOfDataForTesting, featuresToPredict=featuresToPredict, functionsToExcludePoints=functionsToExcludePoints)
	trainXs, trainYs, testXs, testYs = reduceDataToBestFeatures(trainXs, trainYs, testXs, testYs, numFeaturesToKeep=numFeaturesToKeep, predictionAlgorithms=predictionAlgorithms, featuresToPredict=featuresToPredict)
	bestFeatures, accuraciesHash, parametersHash = getPredictionAccuracies(trainXs, trainYs, testXs, testYs, predictionAlgorithms=predictionAlgorithms, featuresToPredict=featuresToPredict)
...
def testGetBestFeaturesAndAccuracies():
	...
	numFeaturesToKeep = 3
	bestFeatures, accuraciesHash, parametersHash = getBestFeaturesAndAccuracies(data, numFeaturesToKeep, predictionAlgorithms, fractionOfDataForTesting, featuresToPredict, functionsToExcludePoints)
