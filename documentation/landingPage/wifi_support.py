...
wifi = nimble.createData('Matrix', 'wifi_localization.txt')
wifi.show(None, maxHeight=9)
nimble.showLog()

# Object methods like shuffle and transform can be logged.  We can always
# override the configured setting by specifying `True` or `False` for `useLog`.
wifi.points.shuffle(useLog=False)
wifi.points.transform(lambda pt: abs(pt), useLog=True)
print(wifi[:3, :])
nimble.showLog()

# We only see a new log entry for `transform`, not `shuffle`, which is what we
# expected.

## Randomness
# For those following along, you may have not expected that `shuffle` would
...
nimble.setRandomSeed(None) # disable consistent results
wifi.points.shuffle()
print(wifi[:3, :])
...
# testing sets.
nimble.setRandomSeed(1234)
wifi = nimble.createData('Matrix', 'wifi_localization.txt', name='wifi')
...
# package name 'Custom', let's try `KNNClassifier`. We will cross validate for
...
performance = nimble.trainAndTest('Custom.KNNClassifier', trainX, trainY,
                                  testX, testY,
                                  nimble.calculate.fractionCorrect,
